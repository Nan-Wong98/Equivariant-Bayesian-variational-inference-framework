from model import SFNet, VIRAttResUNetSR
import torch
import os
import numpy
...
def main(args):
    ...
    demosaic_net, ps_net = SFNet(args), VIRAttResUNetSR()
    ...
    for cnt, (idx, mosaic, pan) in enumerate(tqdm.tqdm(tzip(ids, mosaics, pans))):
        ...
        with torch.no_grad():
            ...
            hrms_tensor = ps_net._modules["RNet"](torch.cat((upms_tensor, pan_tensor), 1), None)
            if args.simulate == False:
