...
# from model_ablation_study_v2 import FuseNet
import copy
import argparse
import json
import cv2
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        with torch.no_grad():
            for cnt, data in enumerate(val_dataloader):
                ...
                mosaic_pred = fuse_net.degrade_bdm(fused, msfa_kernel).detach()
                pan_pred = fuse_net.degrade_srf(fused).detach()
                ...
                if args.visual == True and epoch % args.visual_freq == 0 and epoch != 0:
                    ...
                    mosaic_pred_numpy = mosaic_pred.cpu().numpy()[0].transpose(1, 2, 0)
        ...
        if args.record is not False:
            ...
            record.append({"epoch": epoch,
                        ...
                        #    "R_estimate": fuse_net.degrade_srf.spec_res.weight.detach().flatten().cpu().numpy().tolist(),
                           })
