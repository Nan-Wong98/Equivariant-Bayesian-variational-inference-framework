    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        with torch.no_grad():
            for cnt, data in enumerate(val_dataloader):
                ...
                est_u, est_u_res, est_my, est_mz = fuse_net(mosaic, pan, msfa_kernel, inference_flag=True)
                fused = (est_u+est_u_res).detach()

                psnr_avg += quality_index.calc_psnr(hrms, fused).item()

                if not os.path.exists(os.path.join(args.cache_path, "visual")):
                    os.mkdir(os.path.join(args.cache_path, "visual"))
                ...
                cv2.imwrite(os.path.join(args.cache_path, "visual", f"u{epoch}.png"), rgb)

                # for i in range(len(est_u_list)):
                #     est_u_res_numpy = est_u_list[i].detach()[0].permute(1, 2, 0).cpu().numpy()
                #     est_u_res_numpy = est_u_res_numpy[:, :, [0, 1, 13]]
                #     rgb = numpy.clip(numpy.round(est_u_res_numpy / est_u_res_numpy.max((0, 1)) * 255), 0, 255)
                #     if not os.path.exists(os.path.join(args.cache_path, "visual", f"t{epoch}")):
                #         os.mkdir(os.path.join(args.cache_path, "visual", f"t{epoch}"))
                #     cv2.imwrite(os.path.join(args.cache_path, "visual", f"t{epoch}", f"u_{i+1}_.png"), rgb)

                my_numpy = est_my.detach()[0].permute(1, 2, 0).cpu().numpy()
                rgb = numpy.clip(numpy.round(my_numpy * 255), 0, 255)
                cv2.imwrite(os.path.join(args.cache_path, "visual", f"my{epoch}.png"), rgb)

                mz_numpy = torch.nn.functional.pixel_unshuffle(est_mz.detach(), 4)[0].permute(1, 2, 0).cpu().numpy()
                mz_numpy = mz_numpy[:, :, [0, 1, 13]]
                rgb = numpy.clip(numpy.round(mz_numpy * 255), 0, 255)
                cv2.imwrite(os.path.join(args.cache_path, "visual", f"mz{epoch}.png"), rgb)
